...
def init_db():
    ...
    """)
    # Equipment catalog removed
    cursor.execute("DROP TABLE IF EXISTS equipment_catalog")
    conn.commit()
    ...
    try:
        cursor.execute("ALTER TABLE price_items ADD COLUMN mapping_rule TEXT DEFAULT ''")
        conn.commit()
    except sqlite3.OperationalError:
        pass
...
5. STRUCTURAL, CIVIL & PRELIMINARIES:
   ...
   - Site Safety & Preliminaries: EME chain barrier, roof handrail, crane hire, traffic control, and FIM waste management should be captured with full estimator notes."""
    ...
    # Migration 15: Seed default price registry if empty
    cursor.execute("SELECT COUNT(*) as count FROM price_registry")
    if cursor.fetchone()["count"] == 0:
